defmodule ChurchappWeb.FamilyRelationshipTypesLive.EditLive do
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    case Chms.Church.get_family_relationship_type_by_id(id, actor: actor) do
      {:ok, type} ->
        form =
          type
        ...
        # Get existing relationship types for inverse dropdown (excluding current)
        existing_types =
          case Chms.Church.list_family_relationship_types(actor: actor) do
            {:ok, types} -> Enum.reject(types, fn t -> t.id == id end)
            _ -> []
          end

        inverse_options =
          [{"None", ""}] ++
            Enum.map(existing_types, fn t -> {t.display_name, t.name} end)

        {:ok,
         socket
         |> assign(:page_title, "Edit #{type.display_name}")
         |> assign(:type, type)
         |> assign(:form, form)
          ...
          <div class="flex items-center gap-6">
            ...
            <div class="flex-1">
              <h1 class="text-2xl font-bold text-white mb-2">
                Edit {@type.display_name}
              </h1>
              <p class="text-sm text-gray-400">
                Update the family relationship type details
              </p>
            </div>
          </div>
        </div>
        ...
        <.form for={@form} phx-change="validate" phx-submit="save" id="relationship-type-form">
          <div class="p-6 space-y-6">
            <div>
              ...
              <div class="w-full px-4 py-3 text-gray-400 bg-dark-700/50 border border-dark-600 rounded-md font-mono">
                {@type.name}
              </div>
