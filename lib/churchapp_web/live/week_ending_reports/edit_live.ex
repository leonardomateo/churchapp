defmodule ChurchappWeb.WeekEndingReportsLive.EditLive do
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    case load_report(id, actor) do
      {:ok, report} ->
        ...
        # Initialize category amounts from existing entries
        category_amounts = initialize_category_amounts_from_report(categories, report)
        ...
        socket =
          ...
          |> assign(:week_start_date, Date.to_iso8601(report.week_start_date))
          |> assign(:week_end_date, Date.to_iso8601(report.week_end_date))
          |> assign(:report_name, report.report_name || "")
          |> assign(:notes, report.notes || "")
          |> assign(:errors, %{})
  ...
  defp load_report(id, actor) do
    Chms.Church.get_week_ending_report_by_id(id,
      actor: actor,
      load: [category_entries: [:report_category]]
    )
  end
  ...
  defp initialize_category_amounts_from_report(categories_grouped, report) do
    # Create a map of category_id -> amount from existing entries
    existing_amounts =
      report.category_entries
      |> Enum.map(fn entry -> {entry.report_category_id, Decimal.to_string(entry.amount)} end)
      |> Enum.into(%{})

    # Initialize all categories with existing amounts or 0.00
    categories_grouped
    |> Enum.flat_map(fn {_group, cats} -> cats end)
    |> Enum.map(fn cat ->
      amount = Map.get(existing_amounts, cat.id, "0.00")
      {cat.id, amount}
    end)
    |> Enum.into(%{})
  end
  ...
  def handle_event("save", _params, socket) do
    ...
    if map_size(errors) > 0 do
      ...
      case Chms.Church.update_week_ending_report(report, report_attrs, actor: actor) do
        {:ok, updated_report} ->
          # Delete existing entries and recreate
          delete_existing_entries(report, actor)
          create_category_entries(updated_report, socket.assigns.category_amounts, actor)

          {:noreply,
           socket
           |> put_flash(:info, "Report updated successfully")
           |> push_navigate(to: ~p"/admin/week-ending-reports/#{updated_report}")}
  ...
  defp delete_existing_entries(report, actor) do
    report.category_entries
    |> Enum.each(fn entry ->
  ...
  def render(assigns) do
    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="mb-6">
        ...
        >
          <.icon name="hero-arrow-left" class="mr-2 h-4 w-4" /> Back to Report
        </.link>
        <h2 class="text-2xl font-bold text-white">Edit Week Ending Report</h2>
        <p class="mt-1 text-gray-400">
          Update the financial data for this report.
        </p>
      </div>
